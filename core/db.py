...
# 「強制讀取最新資料庫」——設定頁按鈕用
# =============================================================================
def db_file_info() -> dict:
    """
    twse_ohlcv.db **檔案本身**的狀態（大小、最後修改時間），跟上面幾支函式的
    「查詢結果快取」是兩回事——這支讀的是磁碟，不受 ttl_cache 影響，用來在
    按下「強制讀取最新資料庫」前後對照，確認真的拿到新檔案了。
    """
    if not db_available():
        return {"exists": False}
    st = os.stat(config.TWSE_DB_PATH)
    return {
        "exists": True,
        "path": config.TWSE_DB_PATH,
...
def clear_db_cache() -> int:
    """
    強制讓「下一次」對 twse_ohlcv.db 的查詢重新讀檔，不要繼續回傳記憶體裡
    最長快取 1 小時的舊結果。回傳被清掉的函式數量（固定 3）。

    ⚠️ 為什麼需要這支——_connect() 明明每次都開新連線
    ------------------------------------------------
    問題不在連線層：_connect() 確實每次都重新打開檔案，所以磁碟上的資料本身
    從來不會「讀到舊的」。真正卡住的是這三支查詢函式都包了 @ttl_cache
    （歷史資料快取 1 小時、當日最新價／單日 OHLC 各快取 30 秒）。你手動跑完
    `update_db.py` 換掉整個檔案之後，服務不會自動知道「檔案換了」，還是會在
    TTL 到期前一直回傳快取住的舊結果。這支就是設定頁「強制讀取最新資料庫」
    按鈕背後的邏輯：不用等 TTL、也不用整個重啟服務，按一下就能立刻看到新資料。

    刻意只清這三支，不呼叫 `core.cache.clear_all_caches()` 全部清空——
    Fubon／yfinance 那些即時報價快取秒數本來就短，跟著一起清只會讓盤中
    白白多打一輪不必要的 REST，對「重新讀資料庫」這個目的沒有幫助。
    """
    for fn in (download_history_from_db, get_db_latest_price, get_db_ohlc_for_date):
        fn.clear()
    return 3
