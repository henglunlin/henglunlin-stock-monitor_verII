...
class Settings:
    ...
    # 「彙整」推播（LINE／Telegram 定時彙整）要排除的訊號名稱。跟上面的
    # chart_hidden_signal_labels 刻意分開——那個只管 K 線圖畫不畫，這個才管
    # 會不會被送進 _collect_digest_entries()。空清單 = 預設全部訊號都會被推播。
    digest_hidden_signal_labels: list = field(default_factory=list)
