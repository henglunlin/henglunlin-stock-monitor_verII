...
# =============================================================================
# 富邦 REST：加權指數與個股分鐘 K
# 原始碼對照：0_💻_monitor.py 行 424-441（fetch_taiex_intraday）
# =============================================================================
TAIEX_SYMBOL = "IX0001"


@ttl_cache(ttl=15)
def fetch_taiex_intraday(_sdk) -> dict:
    """
    台股加權指數（TSE，代碼 IX0001）的當日即時報價與分鐘走勢。

    `_sdk` 底線開頭 → 不參與快取雜湊（core/cache.py 沿用了 st.cache_data 的語意）。
    TTL 15 秒是原版的值：指數本身沒有必要比這更即時，而 REST 有流量限制。
    """
    reststock = _sdk.marketdata.rest_client.stock
    return {
...
@ttl_cache(ttl=20)
def fetch_intraday_candles(_sdk, code: str) -> dict:
    """
    單一個股的當日分鐘 K。

    ⚠️ 這支**只在使用者點開單檔詳情時呼叫**，絕對不要在 compute_all_rows 裡
    對 193 檔各叫一次——那是 193 個 REST 請求，一定撞 429 Rate limit。
    表格裡那條迷你走勢用的是 AppState 累積的 tick 序列，成本是零。
    """
    reststock = _sdk.marketdata.rest_client.stock
    return reststock.intraday.candles(symbol=code)


def _candle_points(rows: list) -> list:
    """把富邦的 candles 資料整成 [{t, v}]，時間只留 HH:MM。"""
    points = []
    for r in rows or []:
        ts = r.get("date") or r.get("time")
        close = r.get("close")
        if ts is None or close is None:
            continue
        label = str(ts)
        # 富邦回的是 ISO 字串，取出 HH:MM 就好——X 軸不需要日期
        if "T" in label:
            label = label.split("T", 1)[1][:5]
        elif " " in label:
            label = label.split(" ", 1)[1][:5]
        try:
            points.append({"t": label, "v": float(close)})
        except (TypeError, ValueError):
            continue
    return points


def get_taiex_snapshot(manager) -> dict:
    """
    給 /api/taiex 用。任何失敗都回一個帶 reason 的 dict，不拋例外——
    加權指數只是輔助資訊，不該讓整個狀態列因此變紅。
    """
    sdk = getattr(manager, "sdk", None)
    if sdk is None:
        return {"available": False, "reason": "尚未登入富邦帳號，登入後即可顯示加權指數即時走勢。"}
    try:
        data = fetch_taiex_intraday(sdk)
    except Exception as e:
        return {"available": False, "reason": f"加權指數資料取得失敗：{e}"}

    quote = data.get("quote") or {}
    points = _candle_points((data.get("candles") or {}).get("data") or [])
    if not points:
        return {"available": False, "reason": "目前無加權指數分鐘資料（可能尚未開盤或休市）。"}

    last = quote.get("lastPrice") or quote.get("closePrice") or points[-1]["v"]
...
def get_symbol_intraday(manager, symbol: str) -> dict:
    """
    單檔的盤中走勢。兩個來源，依序退避：

      1. 富邦分鐘 K —— 完整涵蓋 09:00 到現在，即使你中午才打開網頁也看得到全天
      2. 服務自己累積的 tick 序列 —— 沒登入或 REST 失敗時的後備，
         只涵蓋「服務啟動之後」的區間

    第 2 層之所以留著：盤前或假日 REST 會回空，但表格裡的迷你走勢仍然應該有東西畫。
    """
    from core.state import get_state
    from core.symbols import symbol_to_code
    ...
    if sdk is not None:
        try:
            resp = fetch_intraday_candles(sdk, symbol_to_code(symbol))
            points = _candle_points((resp or {}).get("data") or [])
            if points:
                return {"symbol": symbol, "source": "富邦分鐘K", "points": points}
        except Exception as e:
            log.debug("%s 分鐘K取得失敗，退回自行累積的序列：%s", symbol, e)

    points = get_state().series_with_time(symbol)
    return {
        "symbol": symbol,
        "source": "服務累積的即時序列" if points else "無資料",
        "points": points,
    }
