...
/** 推播名單的一筆 Telegram 對象。GET /api/push/recipients 回傳 */
export interface PushRecipientTelegram {
  id: string
  label: string
  chat_id: string
  /** 是否收「盤中即時事件」 */
...
/** 推播名單的一筆 LINE 對象。kind 建議用 'group'——見 core/recipients.py 的額度說明 */
export interface PushRecipientLine {
  id: string
  label: string
  target_id: string
...
/** GET /api/push/recipients 的回應 */
export interface PushRecipients {
  /** 有沒有設過 PIN。false 代表目前新增/刪除/修改不需要 PIN */
  pin_set: boolean
  telegram: PushRecipientTelegram[]
  line: PushRecipientLine[]
}

/**
 * twse_ohlcv.db 檔案本身的狀態（不是查詢快取）。
 * GET /api/debug/db 與 POST /api/admin/reload-db 共用這個形狀，
 * 設定頁「強制讀取最新資料庫」按鈕靠前後兩次的 mtime/size 確認真的換了新檔案。
 */
...
export interface Settings {
  ...
  /** K 線圖上完全不顯示的訊號名稱（K 線圖自己的篩選面板勾選） */
  chart_hidden_signal_labels: string[]
  /** 這些訊號只在 K 線圖最新一天顯示，過去的日子不畫（避免洗版） */
  chart_historical_suppress_labels: string[]
  /**
   * 「彙整」推播（LINE／Telegram 定時彙整）要排除的訊號名稱。
   * 跟 chart_hidden_signal_labels 各自獨立：一個只影響 K 線圖上畫不畫，
   * 一個只影響會不會被送進推播內容，兩邊互不干擾。
   */
  digest_hidden_signal_labels: string[]
  rebound_pct: number
