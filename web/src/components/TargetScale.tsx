...
export function TargetScale({ target, price }: { target: TargetInfo | null; price: number }) {
  // 後端把 NaN 轉成 null，所以量尺的必要欄位缺任何一個就不畫
  const usable =
    target !== null &&
    Number.isFinite(target.buy_low) &&
    Number.isFinite(target.buy_high) &&
    Number.isFinite(target.target_price) &&
    Number.isFinite(price)
  if (!target || !usable) return <span className="text-zinc-600 text-xs">—</span>
