...
import type { DbFileInfo, DetectorDebug, FubonDebug, LineDebug, Settings, SignalCatalogItem, WsDebug } from '../types'
import { Modal } from './Modal'
import { PushRecipientsPanel } from './PushRecipientsPanel'
 ...
 * 設定頁分類分頁。
 *
 * 富邦登入本身不在這裡——那是 Toolbar 上獨立的按鈕/浮動視窗，登入是「隨時可能
 * 要優先處理」的動作，不該被埋進某個分頁裡才看得到。這裡分類的是「調校型」設定，
 * 底下這份「📡 富邦 WebSocket 狀態」只是唯讀診斷，歸進「系統連線與效能」分頁無妨。
 */
type TabKey = 'data' | 'display' | 'intraday' | 'notify' | 'system'

const TABS: { key: TabKey; icon: string; label: string }[] = [
  { key: 'data', icon: '📦', label: '資料來源與模式' },
  { key: 'display', icon: '📊', label: '介面顯示與圖表' },
  { key: 'intraday', icon: '⚡', label: '盤中監控與觸發' },
  { key: 'notify', icon: '📨', label: '訊息推播與名單' },
  { key: 'system', icon: '🔌', label: '系統連線與效能' },
]

/**
 * 「強制讀取最新資料庫」。
 *
 * 這顆按鈕實際做的事是清快取，不是重新打開檔案——core/db.py 的 _connect()
 * 本來每次查詢就會重新開檔案，真正卡住的是三支查詢函式都包了 @ttl_cache
 * （歷史資料快取 1 小時、當日最新價／單日 OHLC 各快取 30 秒）。手動跑完
 * update_db.py 換掉整個 twse_ohlcv.db 之後，服務不會自動知道「檔案換了」，
 * 這顆按鈕就是不想等 TTL、也不想整個重啟服務時的手動捷徑。
 *
 * 按下去前後都打一次 /api/debug/db，用檔案大小／最後修改時間有沒有變化
 * 佐證「真的換了新檔案」，不是只顯示一句「已清除快取」就要你相信。
 */
...
export function SettingsDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // 彙整推播的訊號篩選清單：跟主表格「買賣訊號」欄位讀同一份 SIGNAL_REGISTRY，
  // 不會有「設定頁看得到的訊號」跟「彙整訊息裡實際出現的訊號」兜不起來的問題。
  // （K 線圖自己的訊號篩選面板已經搬到 KLineChart.tsx，各自抓各自的一份。）
  useEffect(() => {
    if (!open) return
    api.signalCatalog().then((d) => setSignalCatalog(d.signals)).catch(() => setSignalCatalog(null))
  }, [open])

  const toggleDigestHiddenSignal = useCallback(
    (label: string) => {
      const current = useStore.getState().status?.settings.digest_hidden_signal_labels ?? []
      const hidden = new Set(current)
      if (hidden.has(label)) hidden.delete(label)
      else hidden.add(label)
      patch({ digest_hidden_signal_labels: Array.from(hidden) })
    },
    [patch],
  )
  ...
  return (
    <Modal open={open} onClose={onClose} title="⚙️ 設定" subtitle="設定存在後端，所有裝置共用同一份" size="lg">
      <div className="flex shrink-0 flex-wrap gap-1 border-b border-zinc-800 bg-zinc-900/40 px-3 py-2">
        {TABS.map((t) => (
          <button
            key={t.key}
...
{JSON.stringify(det.sample ?? [], null, 1)}
        ...
        */}
        <Section
          title="📦 彙整（LINE ＋ Telegram 定時彙整）"
          hint="一天在固定時段各推一次，把目前命中的訊號整理成一則訊息。管道總開關在工具列上，這裡調細節與內容。"
        >
          <Check
            on={s.scheduled_push_enabled}
            onChange={(v) => patch({ scheduled_push_enabled: v })}
            label="定時推送模式"
            hint="只在下面列的時段各推一次彙整訊息。關掉就只剩「推送訊號設定」裡的即時 Telegram 事件與 push 指令。"
          />
        ...
        <Section
          title="📨 推送訊號設定"
          hint="選擇哪些訊息會被推播，分成「彙整」跟「即時」兩類分別設定——兩邊用的是不同的訊號詞彙，所以篩選方式也不一樣。"
        >
          <div>
            <p className="mb-0.5 text-[13px] font-medium text-zinc-300">彙整內容篩選</p>
            <p className="mb-2 text-[11px] leading-relaxed text-zinc-600">
              不勾的訊號完全不會出現在彙整訊息裡（一檔如果命中的訊號全部被排除，這檔那次就不推）。
              跟「K 線訊號」分頁圖表右上角的篩選是各自獨立的兩份清單，互不影響。
            </p>
            {signalCatalog === null ? (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1">
                    {signalCatalog.filter((sig) => sig.kind === 'buy').map((sig) => {
                      const hidden = s.digest_hidden_signal_labels.includes(sig.label)
                      return (
                        <label key={sig.key} className="flex cursor-pointer items-center gap-1.5 text-xs">
                          <input
                            type="checkbox"
                            checked={!hidden}
                            onChange={() => toggleDigestHiddenSignal(sig.label)}
          ...
          <div className="mt-4 border-t border-zinc-800 pt-3">
            <p className="mb-0.5 text-[13px] font-medium text-zinc-300">即時訊號篩選</p>
            <p className="mb-2 text-[11px] leading-relaxed text-zinc-600">
              盤中偵測器抓到的瞬間事件，逐則即時推送（只走 Telegram，不等彙整時段）。1 秒偵測線配
              198 檔，LINE 每月 200 則額度一個上午就會用完，所以刻意不走 LINE。即時事件跟彙整訊號是
              不同的兩套詞彙（瞬間拉抬／反彈／漲跌停 vs. 買賣訊號），所以這裡用等級門檻，不是像彙整
              那樣逐項勾選。
            </p>
            <label className="text-xs text-zinc-400">
              最低推播等級
        ...
        <Section title="📇 推播名單（多人）">
          <p className="mb-3 text-[11px] leading-relaxed text-zinc-600">
            上面兩區是「怎麼推、推什麼」，這裡是「推給誰」。名單裡每個對象自己勾要不要收「即時」
            「彙整」，不再受限於 .env 那一個固定對象。
          </p>
          <PushRecipientsPanel />
        </Section>
        </>
