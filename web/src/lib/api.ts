/**
 * REST 客戶端。
 *
 * 兩種執行情境：
 *  1. 本機開發 —— VITE_API_BASE 留空，走同源 /api，由 vite.config.ts 的 proxy
 *     轉發到 localhost:8000。不會踩到 CORS。
 *  2. 正式環境 —— VITE_API_BASE 指向 Render，真的跨來源，由後端的
 *     CORSMiddleware 放行 Vercel 的網域。
 */
import type {
  ChartHistory, DbFileInfo, DetectorDebug, FubonDebug, GithubDebug, IntradaySeries, LineDebug, MarketEvent,
  PushRecipientLine, PushRecipientTelegram, PushRecipients, Row,
  Settings, SignalCatalogItem, Status, SymbolHit, TaiexSnapshot, TargetEntry, WsDebug,
} from '../types'

export const API_BASE = (import.meta.env.VITE_API_BASE ?? '').replace(/\/$/, '')
const APP_TOKEN = import.meta.env.VITE_APP_TOKEN ?? ''

function headers(extra: Record<string, string> = {}): Record<string, string> {
  const h: Record<string, string> = { ...extra }
  // Render 的網址是公開的，沒有這個 header 後端會回 401。
  if (APP_TOKEN) h['X-App-Token'] = APP_TOKEN
  return h
}

async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  const res = await fetch(`${API_BASE}${path}`, {
    ...init,
    headers: headers(
      init.body ? { 'Content-Type': 'application/json', ...(init.headers as object) } : (init.headers as Record<string, string>) ?? {},
    ),
  })
  if (!res.ok) {
    let detail = `HTTP ${res.status}`
    try {
      const body = await res.json()
      if (body?.detail) detail = typeof body.detail === 'string' ? body.detail : JSON.stringify(body.detail)
    } catch {
      /* 回應不是 JSON，就用 HTTP 狀態碼當訊息 */
    }
    throw new Error(detail)
  }
  return res.json() as Promise<T>
}

/**
 * 喚醒後端。
 *
 * Render 免費方案 15 分鐘無流量就休眠，冷啟動大約 1 分鐘。這支會重試到成功，
 * 讓前端可以顯示「喚醒中…」而不是白畫面 —— 這正是把前端放 Vercel（而不是
 * 跟後端一起放 Render）換來的體驗。
 */
export async function wakeBackend(
  onAttempt?: (attempt: number) => void,
  maxAttempts = 40,
): Promise<boolean> {
  for (let i = 1; i <= maxAttempts; i++) {
    onAttempt?.(i)
    try {
      const res = await fetch(`${API_BASE}/api/health`)
      if (res.ok) return true
    } catch {
      /* 還在冷啟動，繼續等 */
    }
    await new Promise((r) => setTimeout(r, 2000))
  }
  return false
}

export const api = {
  status: () => request<Status>('/api/status'),
  rows: () => request<{ rows: Row[] }>('/api/rows'),
  refreshRows: () => request<{ ok: boolean; count: number; rows: Row[] }>('/api/rows/refresh', { method: 'POST' }),
  groups: () => request<{ groups: Record<string, string[]> }>('/api/groups'),
  settings: () => request<Settings>('/api/settings'),
  patchSettings: (patch: Partial<Settings>) =>
    request<Settings>('/api/settings', { method: 'PATCH', body: JSON.stringify(patch) }),
  fubonLogin: (fubon_id: string, password: string, cert_password: string) =>
    request<{ ok: boolean }>('/api/fubon/login', {
      method: 'POST',
      body: JSON.stringify({ fubon_id, password, cert_password }),
    }),
  resubscribe: () => request<{ ok: boolean }>('/api/fubon/resubscribe', { method: 'POST' }),
  taiex: () => request<TaiexSnapshot>('/api/taiex'),
  intraday: (symbol: string) => request<IntradaySeries>(`/api/intraday/${encodeURIComponent(symbol)}`),
  /** K 線訊號圖：days 不傳就用使用者在設定頁調整過的預設值（後端決定） */
  history: (symbol: string, days?: number) =>
    request<ChartHistory>(
      `/api/history/${encodeURIComponent(symbol)}${days ? `?days=${days}` : ''}`,
    ),
  /** K 線圖「訊號篩選」設定頁的勾選清單用 */
  signalCatalog: () => request<{ signals: SignalCatalogItem[] }>('/api/signals/catalog'),
  wsDebug: () => request<WsDebug>('/api/debug/ws'),
  detectorDebug: () => request<DetectorDebug>('/api/debug/detector'),
  fubonDebug: () => request<FubonDebug>('/api/debug/fubon'),
  githubDebug: () => request<GithubDebug>('/api/debug/github'),
  lineDebug: () => request<LineDebug>('/api/debug/line'),
  /** 發一則測試推播。走的是與正式推播完全相同的後端函式 */
  testPush: (channel: 'line' | 'telegram') =>
    request<{ ok: boolean; reason?: string }>('/api/notify/test', {
      method: 'POST',
      body: JSON.stringify({ channel }),
    }),

  events: (limit = 200) =>
    request<{ events: MarketEvent[]; counts: Record<string, number> }>(`/api/events?limit=${limit}`),
  clearEvents: () => request<{ ok: boolean }>('/api/events/clear', { method: 'POST' }),

  searchSymbols: (q: string) =>
    request<{ results: SymbolHit[] }>(`/api/symbols/search?q=${encodeURIComponent(q)}`),
  saveGroups: (groups: Record<string, string[]>) =>
    request<{
      ok: boolean
      message: string
      subscription?: { added: number; removed: number; reconnected?: boolean }
      groups: Record<string, string[]>
    }>('/api/groups', { method: 'PUT', body: JSON.stringify({ groups }) }),
  reloadGroupsFromGithub: () =>
    request<{ ok: boolean; groups: Record<string, string[]> }>(
      '/api/groups/reload-from-github', { method: 'POST' },
    ),

  // ── 推播名單（多人 Telegram / LINE，PIN 保護） ──
  pushRecipients: () => request<PushRecipients>('/api/push/recipients'),
  setPushPin: (new_pin: string | null, old_pin: string | null) =>
    request<{ ok: boolean; pin_set: boolean }>('/api/push/recipients/pin', {
      method: 'POST', body: JSON.stringify({ new_pin, old_pin }),
    }),
  addTelegramRecipient: (
    row: { label: string; chat_id: string; intraday: boolean; digest: boolean }, pin: string | null,
  ) =>
    request<PushRecipientTelegram>('/api/push/recipients/telegram', {
      method: 'POST', body: JSON.stringify({ ...row, pin }),
    }),
  updateTelegramRecipient: (
    id: string,
    patch: Partial<Pick<PushRecipientTelegram, 'label' | 'chat_id' | 'intraday' | 'digest' | 'enabled'>>,
    pin: string | null,
  ) =>
    request<{ ok: boolean }>(`/api/push/recipients/telegram/${encodeURIComponent(id)}`, {
      method: 'PATCH', body: JSON.stringify({ ...patch, pin }),
    }),
  deleteTelegramRecipient: (id: string, pin: string | null) =>
    request<{ ok: boolean }>(`/api/push/recipients/telegram/${encodeURIComponent(id)}/delete`, {
      method: 'POST', body: JSON.stringify({ pin }),
    }),
  addLineRecipient: (
    row: { label: string; target_id: string; kind: string; intraday: boolean; digest: boolean },
    pin: string | null,
  ) =>
    request<PushRecipientLine>('/api/push/recipients/line', {
      method: 'POST', body: JSON.stringify({ ...row, pin }),
    }),
  updateLineRecipient: (
    id: string,
    patch: Partial<Pick<PushRecipientLine, 'label' | 'target_id' | 'kind' | 'intraday' | 'digest' | 'enabled'>>,
    pin: string | null,
  ) =>
    request<{ ok: boolean }>(`/api/push/recipients/line/${encodeURIComponent(id)}`, {
      method: 'PATCH', body: JSON.stringify({ ...patch, pin }),
    }),
  deleteLineRecipient: (id: string, pin: string | null) =>
    request<{ ok: boolean }>(`/api/push/recipients/line/${encodeURIComponent(id)}/delete`, {
      method: 'POST', body: JSON.stringify({ pin }),
    }),

  // ── twse_ohlcv.db 檔案狀態與「強制讀取最新資料庫」 ──
  dbInfo: () => request<DbFileInfo>('/api/debug/db'),
  reloadDb: () => request<{ ok: boolean; cleared: number } & DbFileInfo>('/api/admin/reload-db', { method: 'POST' }),

  targets: () => request<{ targets: Record<string, TargetEntry> }>('/api/targets'),
  saveTargets: (targets: Record<string, TargetEntry>) =>
    request<{ ok: boolean; message: string; pushed: boolean; targets: Record<string, TargetEntry> }>(
      '/api/targets', { method: 'PUT', body: JSON.stringify({ targets }) },
    ),
  reloadTargetsFromGithub: () =>
    request<{ ok: boolean; targets: Record<string, TargetEntry> }>(
      '/api/targets/reload-from-github', { method: 'POST' },
    ),
}

/** WebSocket 網址。token 走 query string —— 瀏覽器原生 WebSocket 不允許自訂 header。 */
export function wsUrl(): string {
  const base = API_BASE || window.location.origin
  const url = new URL('/ws/quotes', base)
  url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'
  if (APP_TOKEN) url.searchParams.set('token', APP_TOKEN)
  return url.toString()
}
