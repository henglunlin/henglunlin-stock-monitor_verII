...
from core import config, db, groups as core_groups, line as core_line, notify, recipients, targets
...
class SettingsPatch(BaseModel):
    ...
    digest_hidden_signal_labels: list[str] | None = None
...
@router.get("/debug/db", dependencies=auth)
async def debug_db():
    """
    twse_ohlcv.db 本身的狀態（大小、最後修改時間）。給設定頁「資料來源與模式」
    分頁的「強制讀取最新資料庫」按鈕用——按下去前後各打一次，用檔案大小／
    修改時間有沒有變化來確認真的換到新檔案了，不是只看到「已清除快取」四個字。
    """
    return db.db_file_info()


@router.post("/admin/reload-db", dependencies=auth)
async def admin_reload_db():
    """
    強制讓下一次查詢重新讀取 twse_ohlcv.db，不用等歷史資料最長 1 小時的快取
    到期，也不用整個重啟服務。詳細原因見 core/db.py::clear_db_cache() 的說明。
    """
    cleared = db.clear_db_cache()
    return {"ok": True, "cleared": cleared, **db.db_file_info()}
...
# -----------------------------------------------------------------------
# 這一區跟上面 /admin/* 一樣掛 X-App-Token（`auth`），但多一道 PIN 檢查——
# 兩層防護的理由不一樣：X-App-Token 擋的是「不是你、猜不到網址的人」，PIN 擋的是
# 「看得到設定頁畫面、但你不想讓他改推播名單的人」（例如手機借朋友看盤）。
# PIN 沒設過（recipients.has_pin() 是 False）時一律放行，讓你自己第一次使用
# 不用先手動生一把 PIN 才能開始加名單。
class PinOnly(BaseModel):
    pin: str | None = None


class TelegramRecipientIn(BaseModel):
    label: str
    chat_id: str
    intraday: bool = False
    digest: bool = True
...
class TelegramRecipientPatch(BaseModel):
    label: str | None = None
...
class LineRecipientIn(BaseModel):
    label: str
    target_id: str
    kind: str = "group"  # "group"（建議）｜"room"｜"user"
    intraday: bool = False
    digest: bool = True
    pin: str | None = None


class LineRecipientPatch(BaseModel):
    label: str | None = None
...
@router.get("/push/recipients", dependencies=auth)
async def get_push_recipients():
    """設定頁「推播名單」表格吃這個。不回傳 pin_hash 本身，只回有沒有設過。"""
    d = recipients.load()
    return {"pin_set": bool(d.get("pin_hash")), "telegram": d["telegram"], "line": d["line"]}


@router.post("/push/recipients/pin", dependencies=auth)
async def set_push_pin(req: SetPinRequest):
    ok = recipients.set_pin(req.new_pin, req.old_pin)
    if not ok:
        raise HTTPException(status_code=403, detail="舊 PIN 不正確")
    return {"ok": True, "pin_set": recipients.has_pin()}


@router.post("/push/recipients/telegram", dependencies=auth)
async def add_telegram_recipient(req: TelegramRecipientIn):
    try:
        row = recipients.add_telegram(req.label, req.chat_id, req.intraday, req.digest, req.pin)
    except PermissionError:
        raise HTTPException(status_code=403, detail="PIN 不正確")
    return row


@router.patch("/push/recipients/telegram/{row_id}", dependencies=auth)
async def patch_telegram_recipient(row_id: str, req: TelegramRecipientPatch):
    patch = req.model_dump(exclude={"pin"}, exclude_none=True)
    try:
        ok = recipients.update_telegram(row_id, patch, req.pin)
    except PermissionError:
        raise HTTPException(status_code=403, detail="PIN 不正確")
    if not ok:
        raise HTTPException(status_code=404, detail="找不到這筆名單")
    return {"ok": True}


@router.post("/push/recipients/telegram/{row_id}/delete", dependencies=auth)
async def delete_telegram_recipient(row_id: str, req: PinOnly):
    try:
        ok = recipients.delete_telegram(row_id, req.pin)
    except PermissionError:
        raise HTTPException(status_code=403, detail="PIN 不正確")
    if not ok:
        raise HTTPException(status_code=404, detail="找不到這筆名單")
...
@router.post("/push/recipients/line", dependencies=auth)
async def add_line_recipient(req: LineRecipientIn):
    try:
        row = recipients.add_line(req.label, req.target_id, req.kind, req.intraday, req.digest, req.pin)
    except PermissionError:
        raise HTTPException(status_code=403, detail="PIN 不正確")
    return row


@router.patch("/push/recipients/line/{row_id}", dependencies=auth)
async def patch_line_recipient(row_id: str, req: LineRecipientPatch):
    patch = req.model_dump(exclude={"pin"}, exclude_none=True)
    try:
        ok = recipients.update_line(row_id, patch, req.pin)
    except PermissionError:
        raise HTTPException(status_code=403, detail="PIN 不正確")
    if not ok:
        raise HTTPException(status_code=404, detail="找不到這筆名單")
...
@router.post("/push/recipients/line/{row_id}/delete", dependencies=auth)
async def delete_line_recipient(row_id: str, req: PinOnly):
    try:
        ok = recipients.delete_line(row_id, req.pin)
    except PermissionError:
        raise HTTPException(status_code=403, detail="PIN 不正確")
    if not ok:
        raise HTTPException(status_code=404, detail="找不到這筆名單")
...
async def line_webhook(request: Request):
    """
    LINE 傳指令進來的入口。認得兩個指令：
      push  —— 強制掃描並推播，只回 LINE
      id    —— 回這個對話的 source_id（個人聊天是 userId，群組是 groupId）。
               這是取得「群組 ID」最省事的方法：把機器人拉進群組，在群組裡
               傳一句 id，機器人會直接回傳這個群組的 ID，複製貼到設定頁的
               推播名單就能用，不用翻 log。
    ...
    handled = []
    for cmd in core_line.extract_commands(payload):
        text, reply_token = cmd["text"], cmd["reply_token"]
        if text == "push":
            hub.trigger_line_push(reply_token)
            handled.append(text)
        elif text in ("id", "groupid", "群組id", "取得id"):
            kind_label = {"group": "群組", "room": "多人聊天室", "user": "個人"}.get(
                cmd["source_type"], cmd["source_type"],
            )
            # 用 reply（不計入每月 200 則額度），不用 push——這則訊息本來就只是
            # 給你自己複製 ID 用，沒必要花掉推播額度。
            core_line.reply_text(
                reply_token,
                f"這個對話的 ID（{kind_label}）：\n{cmd['source_id']}\n\n"
                f"把這串貼到設定頁「推播名單」的 LINE 那欄就能收到推播。",
            )
            handled.append(text)
        # 2026-09-16 改：不認得的文字安靜忽略，不再回「我看不懂…」。
        # 機器人被拉進一般聊天群組後，成員平常聊天的每一句話都會經過這支 webhook，
        # 原本「看不懂就回一句」的立意是「怕你以為機器人死了」，但代價是每則
        # 非指令訊息都會被機器人插話回覆，在真人對話的群組裡非常干擾。
        # push／id 兩個指令本身有明確的回應（推播結果／ID），已經足夠證明機器人
        # 是活的，不需要再靠這個回覆來確認。
