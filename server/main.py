...
async def unhandled_exception_handler(request: Request, exc: Exception):
    ...
    ⚠️ 為什麼需要這個
    ------------------
    Starlette 預設對未處理例外只回一句 "Internal Server Error"，前端只看得到
    「HTTP 500」，完全不知道發生什麼事。實際踩過一次：某檔股票的欄位是 NaN，
    Starlette 的 JSONResponse 用 allow_nan=False 序列化就直接爆掉，
    但畫面上只有一個紅色的「HTTP 500」，得自己去翻後端 log 才找得到。

    現在改成：後端 log 印完整 traceback，前端也拿得到例外型別與訊息。
    這對「先全部寫完再一起測」的開發方式特別重要——出問題時要能一眼定位。
    """
    ...
    return JSONResponse(
        ...
        content={
            ...
            "hint": "完整 traceback 在後端終端機的 log 裡",
        },
    )
...
async def ws_quotes(websocket: WebSocket, token: str = Query(default="")):
    ...
    try:
        await websocket.send_json({
            ...
            # 事件流一併帶上：中途連進來的瀏覽器也要看得到今天已經發生過的事，
            # 不然重新整理一次跑馬燈就空了。
            "events": get_event_bus().recent(limit=200),
        })
        while True:
