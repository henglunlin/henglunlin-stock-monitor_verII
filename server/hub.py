...
class QuoteHub:
    ...
    def _push_events(self, events: list) -> None:
        """
        把夠重要的盤中事件推給名單裡有勾「盤中即時訊號」的對象。

        2026-09-15 之前這裡直接呼叫 `telegram.send_message()`，只會發給 .env
        寫死的那一個 Telegram 對象。現在改成 `notify.push_event()`，會依
        core/recipients.py 那份名單分派給多個 Telegram/LINE 對象——
        `state.settings.tg_push_enabled` / `line_push_enabled` 兩個總開關還是
        在 notify.push_event() 內部檢查，這裡不用重複判斷。
        """
        state = get_state()
        if not (state.settings.tg_push_enabled or state.settings.line_push_enabled):
            return
        if not (telegram.telegram_configured() or line.line_configured()):
            return
        ...
        for e in events:
            ...
            try:
                ...
                notify.push_event(html_text, plain_text)
    ...
    async def _digest_loop(self) -> None:
        ...
        ── 兩條管道的職責分離 ──
        定時彙整  → LINE ＋ Telegram，依 core/recipients.py 名單分派給多個對象
        即時事件  → 一樣依名單分派（_push_events，2026-09-15 起 LINE 也可以選擇
                    收，但預設名單裡沒有任何 LINE 對象勾這項——見該模組的額度警告）
        push 指令 → 只回 .env 設定的那個 Telegram 對象（你在哪下指令就在哪收回覆，
                    這是輪詢 getUpdates 的天然限制，不是名單機制管得到的範圍）
        """
        while self._running:
            try:
                state = get_state()
    ...
    def _collect_digest_entries(self) -> list[dict]:
        ...
        """
        state = get_state()
        hidden_from_digest = set(state.settings.digest_hidden_signal_labels)
        entries: list[dict] = []
        for row in self.latest_rows():
            hits = row.get("signals") or []
            if not hits:
                continue
            if hidden_from_digest:
                # 設定頁「訊息推播與名單」勾掉的訊號：這裡整批濾掉，不進彙整內容。
                # 跟 chart_hidden_signal_labels 是各自獨立的清單——K 線圖畫不畫是一回事，
                # 推不推播是另一回事。一檔股票如果命中的訊號全部被濾掉，就當這檔沒訊號。
                hits = [h for h in hits if h["label"] not in hidden_from_digest]
                if not hits:
                    continue
